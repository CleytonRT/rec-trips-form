document.addEventListener('DOMContentLoaded', () => {
  ...
  const renderEmbarkOptions = () => {
    ...
    (selectedTrip?.embarques || []).slice(0, 4).forEach((embark) => {
      embarkOptions.appendChild(optionCard({
        name: 'embarkLocation',
        value: `${embark.nome} – ${embark.cidade}`,
        title: embark.nome,
  ...
  const validateStep1 = (showErrors = true) => {
    ...
    ['fullName', 'whatsapp', 'birthDate', 'rg', 'cpf'].forEach((id) => {
      ...
      if (!value) {
        ...
        if (len < 10 || len > 11) message = 'Informe 10 a 11 dígitos';
      }
  ...
  const validateStep3 = (showErrors = true) => {
    ...
    if (!accept.checked) {
      ...
      if (showErrors) {
        const error = document.createElement('small');
        error.className = 'error-text';
        error.textContent = 'Você deve aceitar os termos';
  ...
  $('toSubmit').onclick = () => {
    ...
    fetch(scriptURL, { method: 'POST', mode: 'no-cors', body: data }).catch(() => {});
    showStep(4);
  };
  ...
  whatsapp.addEventListener('blur', (event) => {
    ...
    whatsapp.setCustomValidity(len < 10 || len > 11 ? 'Digite 10 a 11 dígitos' : '');
  });

  fetch('json/viagens.json', { cache: 'no-store' })
    .then((response) => {
    ...
    .catch((error) => {
      ...
      destinationOptions.innerHTML = '<p class="load-error">Não foi possível carregar as viagens. Tente recarregar a página.</p>';
